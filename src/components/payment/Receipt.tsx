...
export function Receipt({ data }: ReceiptProps) {
    ...
    return (
      ...
      */}
            ...
            {/* ── Prominent Action Bar ── print:hidden so it won't appear on the printed page */}
            <div className="print:hidden w-full mt-6 bg-white border border-gray-200 shadow-xl rounded-2xl overflow-hidden">
                <div className="flex flex-col sm:flex-row gap-0">
                    {/* PRIMARY: Print */}
                    <button
                        onClick={handlePrint}
                        className="flex-1 flex items-center justify-center gap-3 h-16 bg-orange-500 hover:bg-orange-600 active:bg-orange-700 text-white font-bold text-lg transition-colors"
                    >
                    ...
                    <button
                        ...
                        className="flex-1 flex items-center justify-center gap-3 h-16 border-t sm:border-t-0 sm:border-l border-gray-200 bg-white hover:bg-gray-50 active:bg-gray-100 text-gray-800 font-bold text-lg transition-colors disabled:opacity-60"
                    >
                        <Download className="w-6 h-6 shrink-0" />
                        {isGeneratingPdf ? t('Generating…') : t('Download PDF')}
                    </button>
                </div>
                <p className="text-center text-sm text-gray-500 py-3 border-t border-gray-100 px-4">
                    {t("No printer? Download the PDF and show it at any SUVIDHA kiosk or bank.")}
                </p>
            </div>
        </div>
    );
